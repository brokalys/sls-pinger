...
const fileName = 'previous-date-3.txt';
...
.then((date) => {
  ...
  const query = `
    ...
    WHERE created_at > ? 
      ...
      AND floor >= 2
      AND area <= 30
      AND price >= 400 AND price <= 700
      AND ST_Contains(ST_GeomFromText('POLYGON((57.04185 24.03191, 57.01495 23.97973, 56.96758 23.93558, 56.9105 23.91792, 56.8405 24.02684, 56.81892 24.12186, 56.82493 24.19121, 56.86024 24.23103, 56.89363 24.29008, 56.92549 24.29077, 56.96726 24.21147, 56.99715 24.17704, 57.04185 24.03191))'), point(lat, lng))
    ORDER BY created_at
  `;
  ...
  return Q.all(results.map((result) => {
    ...
    fs.readFile('email.html', 'utf8', (err, content) => {
      ...
      var data = {
        ...
        subject: '3. kategorija: Jauns PINGER sludinājums',
        html,
      };
